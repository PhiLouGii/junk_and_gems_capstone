...
app.post("/api/users/:userId/purchase-gems", authenticateToken, async (req, res) => {
  ...
  try {
    console.log(`💎 Processing gem purchase for user ${userId}`);
    console.log(`Package: ${package_id}, Gems: ${gems_amount}, Price: M${price}`);

    // Validate input
    if (!gems_amount || !price) {
      return res.status(400).json({ error: "Missing required fields" });
    }
    ...
    try {
      ...
      console.log(`✅ Gems purchase successful: ${currentGems} → ${newBalance}`);
