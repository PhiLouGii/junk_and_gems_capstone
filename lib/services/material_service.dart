...
class MaterialService {
  ...
  static Future<bool> createMaterial(
    ...
    try {
      ...
      print('🖼️ Images received: ${images.length}');
        ...
        if (validImages.isEmpty) {
          ...
          try {
            ...
            if (imageUrls.isEmpty) {
              ...
              print('⚠️ Material will be created WITHOUT images');
        ...
        try {
          ...
          if (responseData['image_urls'] != null) {
            ...
            if (storedUrls.isEmpty && imageUrls.isNotEmpty) {
              print('   ⚠️ WARNING: We sent ${imageUrls.length} URLs but backend stored 0!');
            } else if (storedUrls.length != imageUrls.length) {
              print('   ⚠️ WARNING: Mismatch - sent ${imageUrls.length}, stored ${storedUrls.length}');
            ...
            print('⚠️ WARNING: No image_urls or image_data_base64 in response');
  ...
  static Future<bool> updateMaterial(
    ...
    try {
      print('=' * 80);
      print('📝 UPDATING MATERIAL ID: $materialId');
      print('=' * 80);
      print('📦 Material data received:');
      print('   Title: ${materialData['title']}');
      print('   Category: ${materialData['category']}');
      ...
      // STEP 1: Upload images to Cloudinary (if any provided)
      List<String> imageUrls = [];
      ...
      if (images.isNotEmpty) {
        ...
        if (validImages.isEmpty) {
          print('\n⚠️ WARNING: No valid images found after verification!');
          print('Material will be updated WITHOUT new images...');
        } else {
          print('\n📤 Uploading ${validImages.length} valid images...');
          ...
          try {
            ...
            if (imageUrls.isEmpty) {
              print('\n❌ ERROR: All image uploads failed!');
              print('⚠️ Material will be updated WITHOUT new images');
            } else if (imageUrls.length < validImages.length) {
              print('\n⚠️ WARNING: Some images failed to upload');
              print('✓ Successfully uploaded URLs:');
              for (int i = 0; i < imageUrls.length; i++) {
            ...
            print('\n❌ CLOUDINARY UPLOAD EXCEPTION: $uploadError');
            print('Stack trace: ${StackTrace.current}');
            print('⚠️ Continuing without new images...');
          }
        }
        
        print('─' * 80);
      } else {
        print('\nℹ️ No new images provided for this update');
      }
      
      // STEP 2: Prepare material data
      print('\n📋 PREPARING UPDATE DATA FOR BACKEND...');
      final Map<String, dynamic> requestData = {
        'title': materialData['title']?.toString() ?? '',
        'description': materialData['description']?.toString() ?? '',
        'category': materialData['category']?.toString() ?? '',
      ...
      // Only include image_urls if we have new images
      if (imageUrls.isNotEmpty) {
        requestData['image_urls'] = imageUrls;
        print('📸 Including ${imageUrls.length} new image URLs in update');
      } else {
        print('📸 No image URLs to update (keeping existing images)');
      }

      // STEP 3: Send to backend
      print('\n🚀 SENDING UPDATE TO BACKEND...');
      print('   URL: $_baseUrl/materials/$materialId');
      ...
      if (response.statusCode == 200) {
        ...
        try {
          final errorData = json.decode(response.body);
          throw Exception(errorData['error'] ?? 'Failed to update material');
        } catch (e) {
          throw Exception('Backend error ${response.statusCode}: ${response.body}');
        }
      }
    } catch (e, stackTrace) {
      print('\n❌ UPDATE MATERIAL ERROR: $e');
      print('Stack trace:');
      print(stackTrace);
      print('=' * 80);
      rethrow;
